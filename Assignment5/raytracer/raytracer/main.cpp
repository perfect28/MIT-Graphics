...
int main(int argc, char* argv[])
{
	...
	rayTracer = new RayTracer(sceneParser, max_bounces, weight, shadow, grid,NULL);
	RayTree *rayTree = new RayTree();
	if (gui){
		GLCanvas* glCanvas = new GLCanvas();
		//renderFunction:渲染函数，直接绘制出图像
		//traceRayRunction:光线跟踪函数，预览图中绘制光线路径
		glCanvas->initialize(sceneParser, renderFunction, traceRayFunction,grid,visualize_grid);
	}
	else{
		renderFunction();
	}
