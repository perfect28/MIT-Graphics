...
Ray OrthographicCamera::generateRay(Vec2f point)
{
	...
	float y = point.y();
	Vec3f screen_up;
	Vec3f::Cross3(screen_up, hor, dir);
	screen_up.Normalize();
	Vec3f ori = center + (x - 0.5)*size*hor + (y - 0.5)*size*screen_up;
...
PerspectiveCamera::PerspectiveCamera(Vec3f cer, Vec3f &direction, Vec3f &up, float angle)
{
	...
	this->hor.Normalize();
	this->angle = angle;
	float theta = angle / 2.0f;//该角度已经是弧度
	this->dis = 1.0f / (tan(theta) * 2.0f);
}
...
{
	//这里起点并不能从屏幕出发，因为假设的屏幕很有可能已经超过物体本身了
	float x = point.x();
	float y = point.y(); 
	Vec3f screen_up;
	Vec3f::Cross3(screen_up, hor, dir);
	screen_up.Normalize();
	Vec3f ori = center + (x - 0.5f)*hor + (y - 0.5f)*screen_up + dis*dir;
	Vec3f now_dir = ori - center;
	now_dir.Normalize();
	return Ray(center, now_dir);
}
...
{
	printf("正交相机：\n");
	printf("center:"); center.Write();
	printf("up:"); up.Write();
	printf("dir:"); dir.Write();
	printf("hor:"); hor.Write();
	printf("size:%lf\n", size);
}
